...
s0_1 = 0.387*ae, 0, 0, v/np.sqrt(0.387), 3, 12
s0_2 = 0.723*ae, 0, 0, v/np.sqrt(0.723), 4, 23
s0_3 = 1*ae, 0, 0, v, 6, 2
s0_4 = 1.52*ae, 0, 0, v/np.sqrt(1.52), -5, 3
s0_5 = 2*ae, 0, 0, v/np.sqrt(2), -3, 15
s0_6 = 2.57*ae, 0, 0, v/np.sqrt(2.57), -6, 27

def move_func(s, t):
    x, v_x, y, v_y, q, m = s
    dx_dt = v_x
    dvx_dt = - (k * Q * abs(q) * x / m) / (x**2 + y**2)**1.5
    dy_dt = v_y
    dvy_dt = - (k * Q * abs(q) * y / m) / (x**2 + y**2)**1.5
